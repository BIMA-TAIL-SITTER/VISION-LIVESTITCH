...
Cara Jalankan:
  python3 stitcher.py --session nova_flight_1
  python3 stitcher.py --session nova_flight_1 --batch 5 --gps-threshold 3.0
=============================================================================
"""
...
try:
    ...
    try:
        # Jika dijalankan dari folder PROGRAM-NOVA langsung
...
class StitchingEngine:
    ...
    def stitch(self, images: List[np.ndarray],
        ...
        if STITCHER_AVAILABLE:
            return self._stitch_nova(images, gps_list)
        else:
            return self._stitch_opencv(images)

    def _stitch_nova(self, images: List[np.ndarray],
                     gps_list: List[dict]) -> Optional[np.ndarray]:
        """Gunakan Combiner dari project utama (NOVA)."""
        try:
            log.info(f"Menjalankan NOVA Combiner untuk {len(images)} gambar ...")
            ...
            combiner = Combiner.Combiner(images, data_matrix, output="nova_output")
            result   = combiner.create_mosaic()
            log.info("✓ NOVA Combiner selesai")
            return result

        except Exception as e:
            log.error(f"NOVA Combiner error: {e}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="NOVA Stitcher – GPS-threshold stitcher dengan OD overlay"
    )
...
def main():
    ...
    log.info("=" * 60)
    log.info("PROGRAM-NOVA Stitcher v1.0")
    ...
    log.info(f"  NOVA stitcher   : {'✓ Tersedia' if STITCHER_AVAILABLE else '✗ Fallback OpenCV'}")
    log.info("=" * 60)
