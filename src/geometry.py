...
def warpPerspectiveWithPadding(image, transformation):
    ...
    warped_corners = cv2.perspectiveTransform(corners, transformation) # warped corner locations
    [x_min, y_min] = np.int32(warped_corners.min(axis=0).ravel() - 0.5) # new dimensions
    [x_max, y_max] = np.int32(warped_corners.max(axis=0).ravel() + 0.5)
    translation = np.array(([1,0,-1*x_min],[0,1,-1*y_min],[0,0,1])) # must translate image so that all of it is visible
    full_transformation = np.dot(translation,transformation) # compose warp and translation in correct order
