...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Ground Launcher – jalankan receiver + stitcher sekaligus"
    )
...
def main():
    ...
    log.info("=" * 60)
    log.info("PROGRAM-SENDER Ground Station Launcher")
    ...
    try:
        ...
        p_recv = subprocess.Popen([
            PYTHON, os.path.join(BASE_DIR, "receiver.py"),
        ...
        p_stitch = subprocess.Popen([
            PYTHON, os.path.join(BASE_DIR, "stitcher.py"),
            "--session",       args.session,
        ...
        if args.simulate_od:
            ...
            p_od = subprocess.Popen([
                PYTHON, os.path.join(BASE_DIR, "od_simulator.py"),
                "--host",     args.od_host,
                "--od-port",  str(args.od_port),
                "--interval", "10",
            ])
