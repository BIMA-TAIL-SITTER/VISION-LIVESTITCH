...
def embed_gps_exif(jpeg_bytes: bytes, lat: float, lon: float, alt_m: float) -> bytes:
    """
    Sisipkan koordinat GPS (lat/lon/altitude) ke EXIF gambar JPEG.
    Dipakai stitcher.py di ground buat baca posisi tiap gambar (GPS threshold filter
    & perhitungan ground footprint), jadi TIDAK PERLU dikirim terpisah.
    """
    gps_ifd = {
    ...
    try:
        while True:
            ...
            if embed_gps and gps is not None:
                if gps["has_fix"]:
                    try:
                        jpeg_bytes = embed_gps_exif(
                            jpeg_bytes, gps["latitude"], gps["longitude"], gps["altitude"]
                        )
                    except Exception as e:
                        log.warning(f"Gagal embed GPS EXIF: {e}")
                else:
                    no_gps_warned += 1
                    if no_gps_warned % 10 == 1:
                        log.warning("GPS belum fix — frame dikirim TANPA GPS EXIF.")
